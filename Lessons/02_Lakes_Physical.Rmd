...
#read.csv is different from read_csv, both do same thing
#read_csv is faster
#to read in a file, use ". to go into a folder in the working directory
#relative file path is relative to working directory
#here package does something similar
...
#use $ to designate column here - using base R
NTLdata$sampledate <- as.Date(NTLdata$sampledate, format = "%m/%d/%y")
class(NTLdata$sampledate)
# What does this column look like now?
# have to provide format before change (format above), and need to specify 2 vs 4-digit year
#lowercase y is 2-digit, uppercase y is 4-digit


# Remove rows that contain NAs in temperature column
dim(NTLdata)
NTLdata <- NTLdata %>%
  #%>% is pipe - links things together like "then" in sentence
  drop_na(temperature_C)
# don't need $ here because using dplyr package and just need column name
#alternative is NTLdata <- drop_na(NTLdata, temperature_C)
# Pipe allows another line to be added easily instead of all in one line
dim(NTLdata)


# How many observations are there for each lake?
summary(NTLdata$lakename)
#back to the $ to identify the column
summary(as.factor(NTLdata$lakename))
#gives a count for how many temperature data points there are since we removed
#NA's in temp column

# Let's choose the two lakes with the most data
NTLdata_PeterPaul <- NTLdata %>%
  filter(lakename %in% c("Paul Lake", "Peter Lake"))
# new dataset with just the most sampled lakes
# c concatenates a list
# %in% means includes
...
# within ggplot need to specify aesthetics
# color, shape, etc
# use a plus instead of a pipe with ggplot to indicate to continue on the next line
# alpha is transparency command
...
<add notes here>
Upper water column - epilimnion
Lower water column - hypolimnion
Water is densest at 4 degrees celsius
dimictic lake means mixes twice during the year (spring and fall)
...
thermal differences as weather warms and cools
<add notes here>
Monomictic mixes once a year - happens when there is less thermal stratification. This is the winter because in the summer there is greater differenc in thermal mass between epi and hypolimnion
Monomictic are more likely without iceover in the winter
Polymictic - mixes several times/year - don't see strong seasonal stratification, its more commonly associated with weather events; shallower lakes
amictic - doesn't mix; deep, sheltered lakes
Can see the DCM right at thermocline by looking at DO plot; balance between light and nutrient availability
...
<add notes here>
Thermocline deepens and becomes more consistent over the course of the year
Deeper lakes tend to have deeper and more stable thermoclines
