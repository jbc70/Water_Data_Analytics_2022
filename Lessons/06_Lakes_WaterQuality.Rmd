...
options(scipen = 10) 
#sets number of digits to run before showing scientific notation
...
#huge database, will be expanded to whole US soon from just the NE now
#see vignettes about LAGOSNE for more information
...
# link locus and state data
# join commands typically connect if the same column on separate sheets but 
# need to specify if named differently
# example by = c("columnname1" = "columnname2" )
...
# for left join, start with (., after pipe to specify first dataframe since it needs two dataframes to function)
#drop_na can be everywhere () or specified (col1, col2, etc)
...
#case_when creates a new column when each requirement is met - like an ifelse, but creates something rather than just sorts
...
ggplot(LAGOStrophic, aes(x = state, y = TSI.chl)) +
  ...
  # geom_rect()one way to fill in background layers/levels or
  # geom_density to fill in just the violins
...
# position_dodge shows all the bars side-by-side rather than stacked
# preserve = single makes all the bars the same width no matter how many per state
```
