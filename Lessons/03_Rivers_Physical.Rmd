...
```
For discharge need volume and velocity, but hard to measure so create a rating curve. 
...
#This dataretrieval site is good to look at for what data to look for and subsequently ask for, what parameters are in the set.
...
00001, 00002, and 00003 are daily max, min, mean statistics
>
...
Might be helpful to add same scale for all of them
Specify last plot as taller to have same space as the other plots
Specify to align axes
>
...
Verde is ephemeral - flow with precip, but none when there isnt
...
BitterrootDischarge.Historical <- mutate(BitterrootDischarge.Historical, 
                                         DOY = yday(Date))
#yday function gives day of year (DOY)
...
# y aesthetic separated out for each geom_line, because y is different but x is the same
```
