...
> Not really a good trend, lots of noise/variability. Potentially seasonal trend, but not a strong one. Maybe slightly increasing trend for extreme events. Add a log scale, can maybe see more of a trend? 
...
> Yes. Withdrawals from a river, reservoir, or aquifer over time or any sort of change over time that may have a linear progression. Aggregated data can also be good for linear, or it can be used as an exploratory tool.
...
# double brackets are coding convention within ts function
# frequency = 365 for daily, 12 for monthly, etc
# may have to account for leap years
```
...
> Loess is a kind of moving average funtion
...
#here we're really just making sure we have equally spaced data and month works because of the way this data was collected
NeuseCond_processed <- NeuseCond %>%
  ...
  arrange(Date)
# this method of processing works with data that has only one source - would not work if ResultMeasureValue was more than just conductivity
# floor_date rounds down the date, according to month (specified)
...
# approx function approximates data and creates new df
NeuseCond_monthly$Conductivity <- linearinterpolation$y
#above we are telling R to name the y column in the new df Conductivity
...
```{r}
#1 
Neuse_cond_ts <- ts(NeuseCond_monthly[[3]], frequency = 12)

#2
Neuse_cond_decomp <- stl(Neuse_cond_ts, s.window = "periodic")
plot(Neuse_cond_decomp)

#3
# Not really distinct seasonality, since it has such a small magnitude relative to the data/trend, but is maybe significant at 5-10% of magnitude
# gap is super simplified, can probably just ignore it
# alternative is to splice the seasonal trend onto interpolation
