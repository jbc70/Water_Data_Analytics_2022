...
> aggregation: Combining data into larger sets to see bigger-picture trends or changing scale (daily to monthly) to cover gaps or analyze on different level. Larger scale may have less chance of autocorrelation (monthly data is likely not autocorrelated where daily data probably is)

> subsampling: Taking a smaller piece to examine for statistical analysis, zooming in rather than zooming out. OR same time-step, but subsample at different frequency. 

> interpolation: filling in gaps in data w/ surrounding data
...
NeuseFlow_monthly <- NeuseFlow %>%
  mutate(Year = year(Date),
         Month = month(Date)) %>%
  group_by(Year, Month) %>%
  summarise(Discharge_acftmo = sum(Discharge)*1.98347) #convert to ac-ft
...
```{r}
Condplot <-
ggplot(drop_na(NeuseCond_monthly),
...
> Salinization is occurring, mostly in the winter months (smaller p-value) when there is more discharge as well. More that in the summer months there are more low-salinity events than the winter having more high-salinity. 
...
> We can see a monotonic trend - small p-score, still positive so increasing discharge. Generally seeing changes in the later summer and fall months.
...
#autocorrelation decays over time and we see distinct seasonality
pacf(NeuseFlow_timeseries)
#decays much faster, and see some less-distinct seasonality
```
...
> Not great, the forecast is missing the random variability associated with the natural data. 
