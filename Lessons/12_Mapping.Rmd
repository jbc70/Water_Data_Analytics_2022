...
NeuseCoords <- NeuseParams %>%
  ...
  distinct() #creates df with distinct rows instead of repeats
...
#additional way to get a start point
...
subset <- subset_nhdplus(comids = as.integer(NLDI$UT$nhdplus_comid),#should match start_comid
...
# Create data frames
flowline <- subset$NHDFlowline_Network #line segments within watershed
catchment <- subset$CatchmentSP #polygons with 0 or 1 inlet, 1 outlet
waterbody <- subset$NHDWaterbody #anything not a river or stream
...
gages <- st_intersection(gages, catchment) #only gages within the catchment
#st_difference is reverse of st_intersection
```
...
plot_nhdplus("USGS-02089500", streamorder = 3) #can specify minimum stream order
#do not need to pull previous information to run this command, just need site #
...
max(flowline$totdasqkm) #totaldrainage area in square km
