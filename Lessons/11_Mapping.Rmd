...
secchi.spatial.maine <- st_as_sf(secchi.maine, coords = c("long", "lat"), crs = 4326)
```
...
Depth_plot <- 
  ...
  geom_sf(data = secchi.spatial.maine, aes(color = secchi.mean, size = depth), 
          alpha = 0.5) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1) +
  labs(color = "Average Secchi Depth (m)", size = "Max Depth") +
  theme(legend.position = "top")
```
...
```{r}
plot_grid(Area_plot, Depth_plot,
          ncol = 2,
          align = "hv")
...
> Deeper lakes have deeper secchi depths, hard to tell with these aesthetics (limitation of these methods)
