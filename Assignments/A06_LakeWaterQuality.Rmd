...
#3
NCdata_processed$ActivityStartDate <- as.Date(NCdata_processed$ActivityStartDate, format = "%m/%d/%Y")
...
> I think this stressor response probably is steeper in the mountains where there is likely less natural phosphorus in the water and an influx may cause a stronger response compared to the piedmont or coastal plain where agricultural runoff has elevated levels of phosphorus constantly so an influx in P will not cause as steep a reaction in chla. I also anticipate natural lakes to have a stronger response to phosphorus than reservoirs, as reservoirs are generally larger and will need higher amounts of phosphorus to cause a similarly sized or measured reaction.